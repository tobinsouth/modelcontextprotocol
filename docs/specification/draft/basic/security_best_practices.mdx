...
### Silent Local MCP Server Configuration
...
MCP server repositories or other locations, and do not have proper consent mechanisms,
the following attack becomes possible:
...
- **Social engineering**. Users become comfortable running commands labeled as "MCP servers" without scrutiny.
- **System compromise**. Full system access if the executed commands include privilege escalation.

#### Mitigation

If a MCP client supports one-click local MCP server configuration, it **MUST** implement proper consent mechanisms:
